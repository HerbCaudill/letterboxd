...
describe('isValid', () => {
  ...
  it('should validate uncommon words', () => {
    expect(isValid('ABAC')).toBe(true)
    expect(isValid('ZYTHUMS')).toBe(true)
  })
})
...
describe('commonWords', () => {
  ...
  it('should not contain uncommon words', () => {
    expect(commonWords).not.toContain('ABAC')
    expect(commonWords).not.toContain('ZYTHUMS')
  })
})

describe('generateRandomSolution', () => {
  it('should generate consistent results when given a seed', () => {
    const pair = generateRandomSolution('test-1')
    expect(pair).toMatchInlineSnapshot('"SOIL LIQUEFACTION"')
  })
})
